...
func cleanupUser(t *testing.T, userID int64) {
	...
	if _, err := testDB.Exec(`DELETE FROM ensu_chat_messages WHERE user_id = $1`, userID); err != nil {
		t.Fatalf("failed to cleanup ensu chat messages: %v", err)
	}
	if _, err := testDB.Exec(`DELETE FROM ensu_chat_sessions WHERE user_id = $1`, userID); err != nil {
		t.Fatalf("failed to cleanup ensu chat sessions: %v", err)
	}
	if _, err := testDB.Exec(`DELETE FROM ensu_chat_key WHERE user_id = $1`, userID); err != nil {
		t.Fatalf("failed to cleanup ensu chat key: %v", err)
	}
...
func ensureKey(t *testing.T, repo *Repository, userID int64) {
	...
	if err != nil {
		t.Fatalf("failed to upsert ensu chat key: %v", err)
	}
}
...
func TestMessageDiffAndTombstones(t *testing.T) {
	...
	var messageTwoEntry *model.MessageDiffEntry
	for i := range diff {
		if diff[i].MessageUUID == messageTwo {
			messageTwoEntry = &diff[i]
			break
		}
	}
	if messageTwoEntry == nil {
		t.Fatalf("expected message diff to include %s", messageTwo)
	}
	if messageTwoEntry.ParentMessageUUID == nil || *messageTwoEntry.ParentMessageUUID != messageOne {
...
func TestUpsertAndDeleteIdempotency(t *testing.T) {
	...
	if err := testDB.QueryRow(
		`SELECT COUNT(*) FROM ensu_chat_sessions WHERE session_uuid = $1 AND user_id = $2`,
		sessionUUID,
		userID,
	).Scan(&count); err != nil {
	...
	if err := testDB.QueryRow(
		`SELECT COUNT(*) FROM ensu_chat_messages WHERE message_uuid = $1 AND user_id = $2`,
		messageUUID,
		userID,
	).Scan(&count); err != nil {
