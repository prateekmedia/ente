...
mod rust_to_js {
    ...
    const TEST_PLAINTEXT: &[u8] = b"Hello from Rust! This is test data for cross-platform verification.";
    ...
    fn test_secretbox_encrypt_for_js() {
        ...
        let ciphertext = crypto::secretbox::encrypt_with_nonce(TEST_PLAINTEXT, &nonce, &key).unwrap();
        ...
        assert_eq!(ciphertext.len(), TEST_PLAINTEXT.len() + crypto::secretbox::MAC_BYTES);
    }
...
mod public_api_coverage {
    ...
    fn test_secretbox_encrypt_with_nonce() {
        ...
        let nonce = crypto::keys::generate_secretbox_nonce();
        let ciphertext =
            crypto::secretbox::encrypt_with_nonce(b"test", &nonce, &key).unwrap();
    ...
    fn test_blob_decrypt_json() {
        ...
        let decrypted: serde_json::Value =
            crypto::blob::decrypt_json(&encrypted, &key).unwrap();
    ...
    fn test_stream_encrypt_file() {
        ...
        let (key, header) =
            crypto::stream::encrypt_file(&mut source, &mut dest, None).unwrap();
        assert_eq!(key.len(), 32);
...
mod error_handling {
    ...
    fn test_invalid_key_length_blob() {
        ...
        assert!(result.is_err(), "Should error on invalid key length for blob");
    }
    ...
    fn test_invalid_key_length_stream() {
        ...
        assert!(result.is_err(), "Should error on invalid key length for stream");
    }
    ...
    fn test_invalid_key_login_derivation() {
        ...
        assert!(result.is_err(), "Should error with empty key for login derivation");
    }
    ...
    fn test_short_key_login_derivation() {
        ...
        assert!(result.is_err(), "Should error with short key for login derivation");
    }
    ...
    fn test_wrong_key_decryption_fails() {
        ...
        assert!(result.is_err(), "Should error when decrypting with wrong key");
    }
    ...
    fn test_wrong_key_blob_decryption_fails() {
        ...
        assert!(result.is_err(), "Should error when decrypting blob with wrong key");
    }
    ...
    fn test_wrong_key_stream_decryption_fails() {
        ...
        assert!(result.is_err(), "Should error when decrypting stream with wrong key");
    }
...
mod dart_compatibility {
    ...
    fn test_hash_size() {
        ...
        assert_eq!(hash.len(), 64, "Default hash should be 64 bytes (BLAKE2b-512)");
    }
