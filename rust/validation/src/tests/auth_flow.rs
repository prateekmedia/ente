...
use ente_core::auth::{
    KeyDerivationStrength, generate_keys_with_strength, decrypt_secrets,
};
...
fn libsodium_secretbox_open(ciphertext: &[u8], nonce: &[u8], key: &[u8]) -> Option<Vec<u8>> {
    ...
    if result == 0 { Some(plaintext) } else { None }
}
...
fn test_decrypt_secrets() -> bool {
    ...
    let gen_result = generate_keys_with_strength(password, KeyDerivationStrength::Interactive).unwrap();
    ...
    // Decrypt with core
    let result = decrypt_secrets(password, &gen_result.key_attributes, &encrypted_token);
    ...
    let expected_secret = crypto::decode_b64(&gen_result.private_key_attributes.secret_key).unwrap();
...
fn test_recovery_key() -> bool {
    ...
    let enc_master = crypto::decode_b64(
        gen_result.key_attributes.master_key_encrypted_with_recovery_key.as_ref().unwrap()
    ).unwrap();
    let nonce = crypto::decode_b64(
        gen_result.key_attributes.master_key_decryption_nonce.as_ref().unwrap()
    ).unwrap();
...
fn test_password_change() -> bool {
    ...
    let gen_result = generate_keys_with_strength(old_password, KeyDerivationStrength::Interactive).unwrap();
    ...
    let (new_attrs, new_login_key) = ente_core::auth::generate_key_attributes_for_new_password_with_strength(
        &master_key,
        new_password,
